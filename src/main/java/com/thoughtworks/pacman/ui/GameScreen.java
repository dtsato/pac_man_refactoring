...
public class GameScreen {
	...
	private void drawScore(Graphics g) {
		...
		if (board.demo)
			g.drawString(
					"DEMO MODE PRESS ANY KEY TO START A GAME\t High Score: "
							+ board.highScore, 20, 10);
		else
			g.drawString("Score: " + board.currScore + "\t High Score: "
					+ board.highScore, 20, 10);
	}
		...
		for (int i = 0; i < Pacman.GRID_SIZE; i++) {
			for (int j = 0; j < Pacman.GRID_SIZE; j++) {
				if (board.map.hasWall(i, j)) {
					...
					g.fillRect((i + 1) * Pacman.TILE_SIZE, (j + 1)
	...
	private void drawLives(Graphics g) {
		...
		for (int i = 0; i < board.numLives; i++) {
			g.fillOval(Pacman.TILE_SIZE * (i + 1), Pacman.MAX + 5,
					Pacman.TILE_SIZE, Pacman.TILE_SIZE);
		}
		g.drawString("Reset", 100, Pacman.MAX + 5 + Pacman.TILE_SIZE);
		g.drawString("Clear High Scores", 180, Pacman.MAX + 5
				+ Pacman.TILE_SIZE);
	...
	private void fillPellet(int x, int y, Graphics g) {
		g.setColor(Color.YELLOW);
		g.fillOval((x + 1) * Pacman.TILE_SIZE + 8, (y + 1) * Pacman.TILE_SIZE
				+ 8, 4, 4);
	}
